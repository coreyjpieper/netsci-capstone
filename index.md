...
**Communities** - Like the cross-listed courses network, areas of study were split up into five communities, but with a wider disparity in sizes (11.1% - 29.6%). The smallest community here was the green one, and yet again it was the same community that contained the MSCS department.

**Betweeness Centralities** - If we examine the sizes of the node labels we can view the betweeness centralities graphically. There are two competing duos that dominate the betweeness centralities in the network. Take a second and see if you can spot them. Think you found them? The duos I was refering to were Political Science and International Studies versus Biology and Psychology. Given that we have pairs of nodes with high betweeness right next to each other, it may be the case that several shortest paths in this network use the edge between Political Science and International Studies and the one between Biology and Psychology. So these edges might be acting as two bridges that link majors, minors, and concentrations across different sides of the network. These four vertices are also among the top five eigenvector centralities, which we will now discuss. 
...
The thing that immediately jumps out is how much Computer Science is underperforming in terms of eigenvector centrality. This seems to be due in part because the edge weights for Computer Science are concentrated in just a couple edges; the _degree_ of the subjects around it is comparable.
