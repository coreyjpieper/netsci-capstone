...
The two networks of areas of study at Macalester tell very different stories about the relationships between subjects. As noted before, several of the edges with the highest weight in the graduates network did not exist in the cross-listed courses network. In addition, the network of cross-listed courses was dominated by Women's, Gender, and Sexuality Studies; International Studies; American Studies; and Environmental Studies (top four degree, weighted degree, eigenvector, and PageRank). On the other hand, the network of graduates' degrees was dominated by Political Science, Psychology, International Studies, Economics, and Biology (top five betweeness, eigenvector, and top six PageRank). The only commonality between these two leaderboards was International Studies. In general, vertices that played small roles in one network could easily play central roles in the other network. From this we can conclude that the way departments choose to cross-list their courses has little impact on what subjects graduates actually combine; students will study what they want to study.

#### Future Work

In the future, I would like to compare how the networks have changed over time. I was unable to do this with the cross-listed network because the class schedule changed a lot of it's layout after fall 2016, so I would have to adapt the web scraping code for earlier years. Likewise, I could not do this with the double majors/minors/concentrations because I could only find a program for the class of 2020, but I could do some more digging into this.

One of two more long-term goals I had was creating a symmetric difference network which included edges that were present in one network but not the other. This would make it easier to view the differences between both networks. Another idea I thought of was turning the networks into directed networks by having edge weights represent the proportion of classes or sections that are cross-listed out of all of the class or sections in the department. This would give unique edge weights between pairs of vertices because their departments would be of unequal size.
